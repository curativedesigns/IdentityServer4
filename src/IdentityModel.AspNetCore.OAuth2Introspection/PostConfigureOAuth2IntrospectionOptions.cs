...
namespace IdentityModel.AspNetCore.OAuth2Introspection
{
    internal class PostConfigureOAuth2IntrospectionOptions : IPostConfigureOptions<OAuth2IntrospectionOptions>
    {
        ...
        public PostConfigureOAuth2IntrospectionOptions(IHttpClientFactory httpClientFactory,
        ...
        public void PostConfigure(string name, OAuth2IntrospectionOptions options)
        {
            ...
            if (options.EnableCaching && _cache == null)
            {
                throw new ArgumentException(
                    "Caching is enabled, but no IDistributedCache is found in the services collection", nameof(_cache));
            }
        ...
        private async Task<HttpClient> InitializeIntrospectionClient(OAuth2IntrospectionOptions options)
        {
            if (!options.IntrospectionEndpoint.IsPresent())
            {
                options.IntrospectionEndpoint =
                    await GetIntrospectionEndpointFromDiscoveryDocument(options).ConfigureAwait(false);
            }
                ...
                switch (disco.ErrorType)
                {
                    case ResponseErrorType.Http:
                        throw new InvalidOperationException(
                            $"Discovery endpoint {options.Authority} is unavailable: {disco.Error}");
                    case ResponseErrorType.PolicyViolation:
                        throw new InvalidOperationException(
                            $"Policy error while contacting the discovery endpoint {options.Authority}: {disco.Error}");
                    case ResponseErrorType.Exception:
                        throw new InvalidOperationException(
                            $"Error parsing discovery document from {options.Authority}: {disco.Error}");
                }
            }
