...
internal class DeviceAuthorizationEndpoint : IEndpointHandler
{
    ...
    private async Task<IEndpointResult> ProcessDeviceAuthorizationRequestAsync(HttpContext context)
    {
        ...
        // create response
        _logger.LogTrace("Calling into device authorize response generator: {type}",
    ...
    private void LogResponse(DeviceAuthorizationResponse response,
        DeviceAuthorizationRequestValidationResult requestResult)
    {
        var clientId =
        ...
        {
            _logger.LogTrace("Verification URI issued for {clientId}: {verificationUri}", clientId,
                response.VerificationUri);
        }

        if (response.VerificationUriComplete != null)
        {
            _logger.LogTrace("Verification URI (Complete) issued for {clientId}: {verificationUriComplete}", clientId,
