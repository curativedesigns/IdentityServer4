...
internal class IntrospectionEndpoint : IEndpointHandler
{
    ...
    private async Task<IEndpointResult> ProcessIntrospectionRequestAsync(HttpContext context)
    {
        ...
        if (body == null)
        {
            ...
            await _events.RaiseAsync(
                new TokenIntrospectionFailureEvent(apiResult.Resource.Name, "Malformed request body"));
        ...
        if (validationResult.IsError)
        {
            ...
            await _events.RaiseAsync(
                new TokenIntrospectionFailureEvent(apiResult.Resource.Name, validationResult.Error));

            return new BadRequestResult(validationResult.Error);
        }

        // response generation
        _logger.LogTrace("Calling into introspection response generator: {type}",
    ...
    {
        _logger.LogInformation("Success token introspection. Token active: {tokenActive}, for API name: {apiName}",
            tokenActive, apiName);
    }
