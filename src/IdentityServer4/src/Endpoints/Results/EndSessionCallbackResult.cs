...
internal class EndSessionCallbackResult : IEndpointResult
{
    ...
    private void AddCspHeaders(HttpContext context)
    {
        if (_options.Authentication.RequireCspFrameSrcForSignout)
        {
            ...
            context.Response.AddStyleCspHeaders(_options.Csp, "sha256-u+OupXgfekP+x/f6rMdoEAspPCYUtca912isERnoEjY=",
                frameSources);
        }
    }
    ...
    private string GetHtml()
    {
        ...
        {
            var frameUrls =
                _result.FrontChannelLogoutUrls.Select(url =>
                    $"<iframe src='{HtmlEncoder.Default.Encode(url)}'></iframe>");
            framesHtml = frameUrls.Aggregate((x, y) => x + y);
        }

        return
            $"<!DOCTYPE html><html><style>iframe{{display:none;width:0;height:0;}}</style><body>{framesHtml}</body></html>";
    }
}