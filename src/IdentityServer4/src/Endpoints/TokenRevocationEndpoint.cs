...
internal class TokenRevocationEndpoint : IEndpointHandler
{
    ...
    private async Task<IEndpointResult> ProcessRevocationRequestAsync(HttpContext context)
    {
        ...
        _logger.LogTrace("Calling into token revocation request validator: {type}",
            _requestValidator.GetType().FullName);
        ...
        _logger.LogTrace("Calling into token revocation response generator: {type}",
        ...
        if (response.Success)
        {
            ...
            await _events.RaiseAsync(new TokenRevokedSuccessEvent(requestValidationResult,
                requestValidationResult.Client));
        }
        else
        {
