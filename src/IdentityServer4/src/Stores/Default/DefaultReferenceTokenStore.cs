...
public class DefaultReferenceTokenStore : DefaultGrantStore<Token>, IReferenceTokenStore
{
    ...
    public DefaultReferenceTokenStore(
        ...
        : base(IdentityServerConstants.PersistedGrantTypes.ReferenceToken, store, serializer, handleGenerationService,
            logger)
    {
    }
    ...
    {
        return CreateItemAsync(token, token.ClientId, token.SubjectId, token.SessionId, token.Description,
            token.CreationTime, token.Lifetime);
    }
