...
public class DefaultClaimsService : IClaimsService
{
    ...
    ///     Specifies if all claims should be included in the token, or if the userinfo
    ///     endpoint can be used to retrieve them
    /// </param>
    /// <param name="request">The raw request</param>
    /// <returns>
    ///     Claims for the identity token
    /// </returns>
    public virtual async Task<IEnumerable<Claim>> GetIdentityTokenClaimsAsync(ClaimsPrincipal subject,
        ResourceValidationResult resources, bool includeAllIdentityClaims, ValidatedRequest request)
    {
    ...
    /// </returns>
    public virtual async Task<IEnumerable<Claim>> GetAccessTokenClaimsAsync(ClaimsPrincipal subject,
        ResourceValidationResult resourceResult, ValidatedRequest request)
    {
        ...
        {
            Logger.LogDebug("Client {clientId} is impersonating {impersonatedClientId}", request.Client.ClientId,
                request.ClientId);
        }
        ...
        foreach (var scope in resourceResult.RawScopeValues.Where(x =>
