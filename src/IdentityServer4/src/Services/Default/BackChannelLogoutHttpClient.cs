...
public class DefaultBackChannelLogoutHttpClient : IBackChannelLogoutHttpClient
{
    ...
    public async Task PostAsync(string url, Dictionary<string, string> payload)
    {
        try
        {
            ...
            {
                _logger.LogDebug("Response from back-channel logout endpoint: {url} status code: {status}", url,
                    (int)response.StatusCode);
            }
            else
            {
                _logger.LogWarning("Response from back-channel logout endpoint: {url} status code: {status}", url,
                    (int)response.StatusCode);
            }
        }
        catch (Exception ex)
