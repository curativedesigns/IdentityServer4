...
internal class TokenRequestValidator : ITokenRequestValidator
{
        ...
        NameValueCollection parameters)
    {
        ...
        if (authZcode.ClientId != _validatedRequest.Client.ClientId)
        {
            LogError("Client is trying to use a code from a different client",
                new { clientId = _validatedRequest.Client.ClientId, codeClient = authZcode.ClientId });
            return Invalid(OidcConstants.TokenErrors.InvalidGrant);
        }
        ...
        NameValueCollection parameters)
    {
        ...
        /////////////////////////////////////////////
        if (!await ValidateRequestedScopesAsync(parameters, true, true))
        {
        ...
        bool ignoreImplicitIdentityScopes = false, bool ignoreImplicitOfflineAccess = false)
    {
        ...
        if (scopes.IsMissing())
        {
            ...
            if (!_validatedRequest.Client.AllowedScopes.IsNullOrEmptyEnumerable())
            {
    ...
    {
        return _events.RaiseAsync(new UserLoginFailureEvent(userName, error, false, clientId));
    }
}