...
public class GrantValidationResult : ValidationResult
{
    ...
    ///     Warning: the principal needs to include the required claims - it is recommended to use the other constructor that
    ///     does validation.
    /// </summary>
    ...
    ///     If you want these claims to appear in token, you need to add them explicitly in your custom implementation of
    ///     <see cref="Services.IProfileService" /> service.
    /// </param>
        ...
        Dictionary<string, object> customResponse = null)
    {
        ...
        var resultClaims = new List<Claim>
        {
            ...
            new(JwtClaimTypes.AuthenticationTime, new DateTimeOffset(authTime).ToUnixTimeSeconds().ToString(),
                ClaimValueTypes.Integer64)
        };

        if (!claims.IsNullOrEmptyEnumerable())
        {
