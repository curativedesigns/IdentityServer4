...
public static class CryptoHelper
{
    ...
    internal static X509Certificate2 FindCertificate(string name, StoreLocation location, NameType nameType)
    {
        ...
        if (location == StoreLocation.LocalMachine)
        {
            ...
            {
                certificate = X509.LocalMachine.My.SubjectDistinguishedName.Find(name, false).FirstOrDefault();
            }
            else if (nameType == NameType.Thumbprint)
            {
                certificate = X509.LocalMachine.My.Thumbprint.Find(name, false).FirstOrDefault();
            }
        }
        else
        {
            if (nameType == NameType.SubjectDistinguishedName)
            {
                certificate = X509.CurrentUser.My.SubjectDistinguishedName.Find(name, false).FirstOrDefault();
            }
            else if (nameType == NameType.Thumbprint)
            {
                certificate = X509.CurrentUser.My.Thumbprint.Find(name, false).FirstOrDefault();
            }
        }
