...
public class AuthorizeInteractionResponseGenerator : IAuthorizeInteractionResponseGenerator
{
        ...
        ConsentResponse consent = null)
    {
        ...
        if (consent != null && consent.Granted == false && consent.Error.HasValue &&
            request.Subject.IsAuthenticated() == false)
        {
        ...
        if ((result.IsLogin || result.IsConsent || result.IsRedirect) &&
    ...
    protected internal virtual async Task<InteractionResponse> ProcessLoginAsync(ValidatedAuthorizeRequest request)
    {
        ...
        {
            Logger.LogInformation("Showing login: request contains prompt={0}",
                request.PromptModes.ToSpaceSeparatedString());
        ...
        if (isAuthenticated)
        {
            var isActiveCtx = new IsActiveContext(request.Subject, request.Client,
                IdentityServerConstants.ProfileIsActiveCallers.AuthorizeEndpoint);
                 ...
                 !request.Client.IdentityProviderRestrictions.Contains(currentIdp))
        {
            Logger.LogInformation(
                "Showing login: User is logged in with idp: {idp}, but idp not in client restriction list.",
                currentIdp);
            return new InteractionResponse { IsLogin = true };
        }
        ...
        if (request.Client.UserSsoLifetime.HasValue)
        {
            ...
            {
                Logger.LogInformation(
                    "Showing login: User's auth session duration: {sessionDuration} exceeds client's user SSO lifetime: {userSsoLifetime}.",
                    diff, request.Client.UserSsoLifetime);
                return new InteractionResponse { IsLogin = true };
            }
        }
    ...
    protected internal virtual async Task<InteractionResponse> ProcessConsentAsync(ValidatedAuthorizeRequest request,
        ConsentResponse consent = null)
    {
        if (request == null)
        {
        ...
        var consentRequired =
            await Consent.RequiresConsentAsync(request.Subject, request.Client,
                request.ValidatedResources.ParsedScopes);
        ...
        if (request.PromptModes.Contains(OidcConstants.PromptModes.Consent) || consentRequired)
        {
            ...
            else
            {
                ...
                if (consent.Granted == false)
                {
                    ...
                    var error = consent.Error switch
                    {
                        AuthorizationError.AccountSelectionRequired => OidcConstants.AuthorizeErrors
                    ...
                    else
                    {
                        ...
                        if (request.Client.AllowRememberConsent)
                        {
                            ...
                            if (consent.RememberConsent)
                            {
                                ...
                                Logger.LogDebug("User indicated to remember consent for scopes: {scopes}",
