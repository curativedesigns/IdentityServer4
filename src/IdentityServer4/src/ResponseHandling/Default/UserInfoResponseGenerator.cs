...
public class UserInfoResponseGenerator : IUserInfoResponseGenerator
{
    ...
    public virtual async Task<Dictionary<string, object>> ProcessAsync(UserInfoRequestValidationResult validationResult)
    {
        ...
        var scopes = validationResult.TokenValidationResult.Claims.Where(c => c.Type == JwtClaimTypes.Scope)
            .Select(c => c.Value);
        ...
        else
        {
            ...
            Logger.LogInformation("Profile service returned the following claim types: {types}",
    ...
    /// <returns></returns>
    protected internal virtual async Task<ResourceValidationResult> GetRequestedResourcesAsync(
    ...
    /// <returns></returns>
    protected internal virtual Task<IEnumerable<string>> GetRequestedClaimTypesAsync(
