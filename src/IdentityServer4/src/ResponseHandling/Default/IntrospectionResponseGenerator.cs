...
public class IntrospectionResponseGenerator : IIntrospectionResponseGenerator
{
    ...
    /// <returns></returns>
    public virtual async Task<Dictionary<string, object>> ProcessAsync(
        IntrospectionRequestValidationResult validationResult)
    {
    ...
    /// <param name="validationResult">The validation result.</param>
    /// <returns></returns>
    protected virtual async Task<bool> AreExpectedScopesPresentAsync(
        IntrospectionRequestValidationResult validationResult)
    {
        ...
        else
        {
            ...
            await Events.RaiseAsync(new TokenIntrospectionFailureEvent(validationResult.Api.Name,
                "Expected scopes are missing", validationResult.Token, apiScopes, tokenScopes.Select(s => s.Value)));
        }
